...
app.patch('/api/registration/:id/verify', async (req, res) => {
  try {
    const { verified, verifiedBy, validationNotes } = req.body;
    ...
    registration.paymentScreenshot.validationNotes = validationNotes || 'Payment verified';
    registration.status = verified ? 'approved' : 'pending';
